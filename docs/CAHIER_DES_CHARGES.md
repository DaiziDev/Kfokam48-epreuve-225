...
| RG9  | Une relecture rendue est définitive, aucune correction possible ensuite                                                            | Q15 (retenue contre Q10, voir la section 7 pour comprendre moin choix) |
...
Java 21+, Maven avec wrapper `mvnw` commité, Spring Boot en couches contrôleur/service/repository, DTO obligatoires, validation des entrées et gestion centralisée des erreurs via `@RestControllerAdvice`, schéma versionné par migrations (Flyway ou Liquibase), format d'erreur imposé `{ code, message }` sur toutes les erreurs. Frontend au choix (React, Angular, Next.js), appels API isolés dans une couche dédiée, aucun recalcul de règle métier côté client.

## 9. Livrables

Dépôt GitHub public `kfokam48-epreuve-225` avec `/docs`, `/api`, `/backend`, `/frontend` ; contrat d'API complété ; trois diagrammes Mermaid versionnés ; backlog en issues ; `CHANGELOG.md` ; `README` d'installation testé depuis un clone vierge ; `JOURNAL.md` tenu à chaque étape.

## 10. Démarche prévue

1. Trancher les contradictions et le trou (fait ci-dessus), compléter le contrat d'API.
2. Rédiger les diagrammes D1/D2/D3 en cohérence avec ce cahier des charges.
